...
static constexpr std::string_view kAllMetrics[] = {"psnr", "psnr-y", "ssim", "ms-ssim", "psnr-hvs", "xpsnr", "xpsnr-y", "vmaf", "ssim2", "butteraugli"};
...
static bool parse_args(Options& opts, int argc, char* argv[]) {
    ...
    if (positional.size() < 2) {
        std::cerr << "Usage: eyeq [--all] [--psnr] [--psnr-y] [--ssim] [--ms-ssim] [--psnr-hvs] [--xpsnr] [--xpsnr-y] [--vmaf] [--ssim2|--ssimulacra2] "
                     "[--butteraugli] "
                     "<reference> <distorted>\n";
        return false;
    }
